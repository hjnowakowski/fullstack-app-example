...
class CourseGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Course entity")
        ...
        .repeat(2) {
            ...
            .get("/armory/api/courses")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new course")
            .post("/armory/api/courses")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "courseTitle":"SAMPLE_TEXT", "courseDescription":"SAMPLE_TEXT", "coursePrice":null, "courseLevel":null}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_course_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created course")
                .get("/armory${new_course_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created course")
            .delete("/armory${new_course_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
